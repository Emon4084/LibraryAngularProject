...
export class CategoryComponent implements OnInit {
  ...
  // pageSize = 5;
  dataSource = new MatTableDataSource<Category>([]);
  displayedColumns: any[] = ['serialNumber', 'categoryName', 'ddcCode', 'isActive','actions'];
  pageSize = 5;
  ...
  updateDataSource(): void {
    ...
    this.categories.slice(startIndex, startIndex + itemsPerPage).forEach((category, index) => {
      const adjustedIndex = startIndex + index + 1;
      (category as any)['serialNumber'] = this.toRoman(adjustedIndex); //please check this lince, Here serialName will be change when do to update
    });
    this.dataSource.data = this.categories;
  }
